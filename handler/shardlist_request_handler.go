...
func InitShardlistRequestHandler() {
	handler := ShardlistRequestHandler{channel: server.PacketManagerInstance.GetQueue(opcode.ShardlistRequest)}
	go handler.Handle()
}

func (h *ShardlistRequestHandler) Handle() {
	for {
		packet := <-h.channel
